...
# %% Stress Post-Processing (MPa)
def compute_stresses(w, l, h, b, I, F_normal, F_shear, num_beams, k_val):
    # F_normal: Axial force (y-direction)
    # F_shear: Transverse force (x-direction)
    
    # 1. Bending Stress: sigma_b = M * c / I
    # M = F_bending * L / 2 (discrete approximation)
    sigma_b = ((k_val * w * l * 0.5 * (h / 2)) / (num_beams * I)) / 1e6 
    
    # 2. Normal Stress: sigma_n = F_normal / A
    sigma_n = (F_normal / (num_beams * b * h)) / 1e6
    
    # 3. Shear Stress: tau = F_shear / A
    tau = (F_shear / (num_beams * b * h)) / 1e6
    
    # 4. Von Mises: sqrt((sigma_bending + sigma_axial)^2 + 3 * tau^2)
    sigma_vm = np.sqrt((sigma_b + sigma_n)**2 + 3 * tau**2)
    
    return sigma_b, sigma_n, tau, sigma_vm

# Updated calls using your direct Rx and Ry variables
S1_b, S1_n, S1_t, S1_vm = compute_stresses(w1, l1, h1, b1, I1, R_I1y, R_I1x, 4, k1)
S2_b, S2_n, S2_t, S2_vm = compute_stresses(w2, l2, h1, b1, I1, R_I2y, R_I2x, 4, k2)
S3_b, S3_n, S3_t, S3_vm = compute_stresses(w3, l3, h1, b1, I1, R_I3y, R_I3x, 2, k3)
S4_b, S4_n, S4_t, S4_vm = compute_stresses(w4, l4, h1, b1, I1, R_I4y, R_I4x, 2, k4)
S5_b, S5_n, S5_t, S5_vm = compute_stresses(w5, l5, h2, b2, I2, R_B55y, R_B55x, 1, k5)
S6_b, S6_n, S6_t, S6_vm = compute_stresses(w6, l6, h2, b2, I2, R_B56y, R_B56x, 1, k6)
...
# Define 6 distinct linestyles for the 6 beams
...
for i, (ax, (data_list, title, unit)) in enumerate(zip(axes, stress_types)):
    for j, data in enumerate(data_list):
        line, = ax.plot(azimuth_plot, data[sort_idx], 
                        linestyle=lstyles[j], linewidth=1.5)
        if i == 0:
            handles.append(line)
            labels.append(f"Beam {j+1}")
    
    ax.set_title(title, fontweight='bold', fontsize=10)
    ax.set_ylabel(f"Stress [{unit}]", fontsize=9)
...
axes[2].set_xlabel("Azimuth Angle [$^{\circ}$]")
axes[3].set_xlabel("Azimuth Angle [$^{\circ}$]")
