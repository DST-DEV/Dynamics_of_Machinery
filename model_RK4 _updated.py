...
if plot_lines:
    ...
    labels1 = [f"Mass{i+1}" for i in range(6)]
    labels =  ["Mass5", "Mass6"]
    ...
    with mpl.rc_context(matlab_rc):

        # --- Plot 1: Beam deflections over time ---
        fig, ax = plt.subplots()
        for i, (data, lbl) in enumerate(zip([ w5, w6], labels)):
            ax.plot(t_int, data, color=colors[i], linestyle="-",
                    linewidth=1.6, label=lbl)
        ax.set_title("Blade deflections over time")
        ...
        # --- Plot 1: Beam deflections over time ---
        fig, ax = plt.subplots()
        for i, (data, lbl) in enumerate(zip([w1, w2, w3, w4], labels1)):
            ax.plot(t_int, data, color=colors[i], linestyle="-",
                    linewidth=1.6, label=lbl)
        ax.set_title("Tower Beam deflections over time")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Deflection (m)")
        ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28),
                  ncol=6, frameon=True)
        ...
        # --- Plot 2: Reaction forces in global x-direction (shear force) over time ---
        fig, ax = plt.subplots()
        fx_data = [F_I1[:, 0], F_I2[:, 0], F_I3[:, 0],
                   F_I4[:, 0], F_I5[:, 0], F_I6[:, 0]]
        for i, (data, lbl) in enumerate(zip(fx_data, labels1)):
            ax.plot(t_int, data, color=colors[i], linestyle="-",
                    linewidth=1.6, label=lbl)
        ax.set_title("Reaction forces in global x-direction (shear force) over time")
        ax.set_xlabel("Time (s)")
        ...
        # --- Plot 3: Reaction forces in global y-direction (normal force) over time ---
        fig, ax = plt.subplots()
        fy_data = [F_I1[:, 1], F_I2[:, 1], F_I3[:, 1],
                   F_I4[:, 1], F_I5[:, 1], F_I6[:, 1]]
        for i, (data, lbl) in enumerate(zip(fy_data, labels1)):
            ax.plot(t_int, data, color=colors[i], linestyle="-",
                    linewidth=1.6, label=lbl)
        ax.set_title("Reaction forces in global y-direction (normal force) over time")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Force (N)")
        ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28),
                  ncol=6, frameon=True)
        ...
        # --- Plot 4: Reaction forces in local x-direction for blade nodes ---
        fig, ax = plt.subplots()
        blade_data  = [F_B55[:, 0], F_B56[:, 0]]
        blade_labels = ["Mass5", "Mass6"]
        for i, (data, lbl) in enumerate(zip(blade_data, blade_labels)):
            ax.plot(t_int, data, color=colors[i], linestyle="-",
                    linewidth=1.6, label=lbl)
        ax.set_title("Reaction forces in local x-direction (shear force)for blade nodes over time")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Force (N)")
        ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28),
                  ncol=2, frameon=True)
        ...
        # --- Plot 5: Reaction forces in local y-direction for blade nodes ---
        fig, ax = plt.subplots()
        blade_data = [F_B55[:, 1], F_B56[:, 1]]
        for i, (data, lbl) in enumerate(zip(blade_data, blade_labels)):
            ax.plot(t_int, data, color=colors[i], linestyle="-",
                    linewidth=1.6, label=lbl)
        ax.set_title("Reaction forces in local y-direction (normal force) for blade nodes over time")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Force (N)")
        ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28),
                  ncol=2, frameon=True)
